...
## ผู้ให้บริการสำรอง และ circuit breaker

ลำดับการถาม: **ไปรษณีย์ไทย → Track123 → ETrackings**

### Circuit breaker

`lib/circuit-breaker.ts` — Track123 พัง **5 ครั้งใน 1 นาที** → ตัดวงจร 30 วินาที
ระหว่างนั้นคำขอถูกปฏิเสธทันทีโดยไม่ยิงและไม่เข้าคิว ผู้ใช้จึงไม่ต้องรอ timeout
20 วินาทีทีละคน และคิวไม่ถูกอุดด้วยคำขอที่รู้อยู่แล้วว่าจะพัง

ครบเวลาพักแล้วจะปล่อยผ่าน **คำขอเดียว** เพื่อลองแตะดู สำเร็จ → กลับมาปกติ ·
พังอีก → พักใหม่ตั้งแต่ต้น
...
| โควตา | **แผนฟรี 50 ครั้ง/เดือน** |

**เรียกเฉพาะตอน Track123 "พัง" เท่านั้น ไม่ใช่ตอนตอบว่า "ไม่พบ"** — ถ้ายิงทุกครั้ง
ที่ค้นไม่เจอ โควตาจะหมดภายในไม่กี่วัน และไม่มีการลองใหม่อัตโนมัติเลยแม้เจอ 429

⚠️ **ข้อจำกัดสำคัญ**: `/tracks/find` บังคับให้ระบุ `courier` ต่างจาก Track123
ที่ตรวจจับเอง ด้วยโควตา 50 ครั้ง/เดือน การไล่เดาทีละเจ้าเป็นไปไม่ได้ (เดาผิด
หนึ่งครั้ง = 2% ของทั้งเดือน) adapter นี้จึงทำงานได้เฉพาะเลขที่ระบุขนส่งได้จาก
`COURIER_PREFIXES` (`lib/carriers/courier-prefix.ts`) — ตอนนี้คือ `SPXTH` เท่านั้น
**การเติมแถวในตารางนั้นคือทางเดียวที่ทำให้เจ้าสำรองครอบคลุมเลขได้มากขึ้น**

ยังไม่ตั้ง `ETRACKINGS_API_KEY` / `ETRACKINGS_KEY_SECRET` → ระบบทำงานเหมือนเดิม
ทุกประการ ตอน Track123 ล่มจะตกไปที่ cache ตามกลไก degradation เดิม

### นับการใช้โควตาจาก pm2

```bash
L='pm2 logs --nostream --lines 20000'

$L | grep -c '\[etrackings\]'        # ยิงไปกี่ครั้ง (บรรทัดมี used=n/50 ด้วย)
$L | grep -c 'via=backup'            # คำค้นที่จบด้วยเจ้าสำรอง
$L | grep '\[breaker\]'              # วงจรถูกตัด/ฟื้นตอนไหน
```

ตัวนับ `used=n/50` อยู่ใน memory ของ process เดียว จึงหายตอน restart และเป็น
"อย่างน้อยเท่านี้" ไม่ใช่ยอดจริง — ยอดจริงดูที่ dashboard ของ ETrackings
