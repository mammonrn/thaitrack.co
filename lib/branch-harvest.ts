 ...
 *
 * 1. **ห้ามเขียนพิกัดระดับ "พื้นที่" ลง carrier_branches** ตารางนั้นคือที่ที่
 *    ทั้งระบบเชื่อว่าถูกต้อง ถ้าปล่อยหมุดกลางอำเภอเข้าไปนั่ง บั๊กเดิม (ดู
 *    migration 0004) จะกลับมาแบบถาวรและมองไม่เห็น จึงปฏิเสธทุกผลที่ติดด่าน
 *    types[]/partial_match (areaOnly) ไม่ว่ากรอบจะเล็กแค่ไหน
 *
 *    ⚠️ ที่ปฏิเสธคือ **ชนิด** ของผลลัพธ์ ไม่ใช่ **ขนาด** — ตั้งแต่ตัดเพดาน
 *    12 กม. ทิ้ง กรอบใหญ่ๆ ที่ไม่ใช่เขตปกครองเขียนลงตารางได้แล้ว โดยติดชั้น
 *    coarse ไว้กับแถวนั้น (คอลัมน์ accuracy รับค่านี้มาตั้งแต่ migration 0009)
 *    ความหยาบจึงไม่ได้ถูกซ่อน มันถูกบันทึกไว้ให้ UI เอาไปขึ้นป้ายต่อ
 *
