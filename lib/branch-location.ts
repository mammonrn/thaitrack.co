...
/**
 * โทเคนที่เป็นอักษรไทยล้วน — มีขีดคั่นได้ เช่น "เวียง-เชียงแสน"
 *
 * ห้ามมีตัวเลข อักษรละติน หรือเครื่องหมายอื่น เพราะสิ่งที่กติกานี้ต้องแยกให้ออก
 * คือ "ชื่อสถานที่" กับ "รหัสภายในของขนส่ง" ซึ่งรหัสมักมีเลขหรือละตินปน
 */
const THAI_TOKEN = /^[\u0E00-\u0E7F]+(?:[-–—][\u0E00-\u0E7F]+)*$/;

/** มีอักษรละตินอย่างน้อยหนึ่งตัว = สัญญาณว่าเป็นรหัสภายใน ไม่ใช่บ้านเลขที่ */
const HAS_LATIN = /[A-Za-z]/;

/**
 * ตัดรหัสสาขาที่ปนอยู่หน้าชื่อสถานที่ออก — "" เมื่อไม่มีอะไรควรตัด
 *
 * ปัญหาที่แก้ (เจอกับ J&T): สถานที่มาเป็น
 *   "สาขา46Chiang Saen01 เวียง-เชียงแสน เชียงราย"
 * ส่วนที่ Google หาเจอจริงคือ "เวียง-เชียงแสน เชียงราย" ส่วนหน้าเป็นรหัสภายใน
 * ที่ไม่มีความหมายกับใครนอกจากขนส่ง ถ้าโยนทั้งก้อนไป ความมั่วของส่วนหน้าจะ
 * ดันให้ผลลัพธ์เพี้ยนหรือกว้างกว่าที่ควร
 *
 * เงื่อนไขที่ต้องครบก่อนตัด — เข้มไว้ก่อน เพราะการตัดผิดคือการทิ้งข้อมูลที่ใช้ได้:
 *   1. ต้องมีโทเคนไทยล้วนต่อท้ายอย่างน้อยสองคำ (คำเดียวแคบเกินกว่าจะเชื่อ
 *      ว่าเป็นชื่อสถานที่ที่หาพิกัดได้ถูกที่)
 *   2. ส่วนหน้าที่จะตัดทิ้งต้องมีอักษรละตินปน — กันไม่ให้ไปตัดบ้านเลขที่
 *      อย่าง "123 ถนนสุขุมวิท กรุงเทพ" ซึ่งตัวเลขข้างหน้าคือข้อมูลจริง
 *   3. ทั้งก้อนเป็นไทยล้วนอยู่แล้ว = ไม่มีอะไรให้ตัด คืน "" ไปตามเดิม
 */
export function thaiPlaceTail(raw: string): string {
  const tokens = tidy(raw).split(" ").filter((token) => token !== "");

  let start = tokens.length;
  while (start > 0 && THAI_TOKEN.test(tokens[start - 1])) start -= 1;

  // ไทยล้วนทั้งก้อน → ไม่มีรหัสให้ตัด
  if (start === 0) return "";
  if (tokens.length - start < 2) return "";
  if (!HAS_LATIN.test(tokens.slice(0, start).join(" "))) return "";
...
export function parseLocationText(raw: string): ParsedLocation {
  ...
  if (looksLikeAddress(value)) {
    // แสดงข้อความเต็มตามที่ขนส่งบอกมา แต่ส่งเฉพาะส่วนที่หาพิกัดได้จริงไปให้
    // Google — สองอย่างนี้ไม่จำเป็นต้องเป็นข้อความเดียวกัน
    const tail = thaiPlaceTail(value);
