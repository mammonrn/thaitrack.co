 ...
 *
 * ══════════════════════════════════════════════════════════════════
 * ⚠️ ตัวนับอยู่ **ในนี้** ไม่ใช่ที่ผู้เรียก
 *
 * ของเดิมนับครั้งเดียวที่ trackOnce ก่อนยิง แล้วพอเจอ 401/403 มันขอ token ใหม่
 * แล้วเรียก requestTracking อีกรอบ — request ที่สองนั้น **ไม่เคยถูกนับ**
 * ตัวนับจึงต่ำกว่าความจริงทุกครั้งที่ token หมดอายุกลางทาง
 *
 * นับต่ำกว่าจริงอันตรายกว่านับเกิน: นับเกิน = ตกใจเกินเหตุแล้วไปตรวจเจอ
 * ส่วนนับต่ำ = คิดว่ายังเหลือเยอะ แล้วชนกำแพงโดยไม่มีสัญญาณเตือนมาก่อนเลย
 *
 * **หลักการ: ตัวนับต้องอยู่ติดกับจุดที่จ่ายเงินจริง ไม่ใช่จุดที่เราคิดว่าจ่าย**
 * ที่นี่คือฟังก์ชันเดียวที่ยิง /track ออกไปจริง ทุกเส้นทางผ่านตรงนี้หมด
 * ══════════════════════════════════════════════════════════════════
 */
...
async function trackOnce(trackingNumber: string): Promise<TrackingResult> {
  ...
  // ⚠️ ไม่นับตรงนี้แล้ว — ตัวนับย้ายไปอยู่ใน requestTracking() ซึ่งเป็นจุดที่
  // ยิงจริง · ของเดิมนับที่นี่ครั้งเดียว แล้วรอบที่สอง (ตอน token หมดอายุ)
  // หลุดการนับไปเงียบๆ
  //
  // เลขที่รูปแบบผิดยังไม่ถูกนับเหมือนเดิม เพราะมันโยน error ทิ้งไปตั้งแต่ข้างบน
  // ก่อนจะมาถึงบรรทัดนี้ ไม่เคยมี request ออกไปถึงปลายทางเลย
