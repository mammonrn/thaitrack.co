 ...
 *
 * ------------------------------------------------------------------
 * ⚠️ **upstream_error เคยอยู่ในชุดนี้ แล้วถูกถอดออกจากการวัดของจริง**
 *
 * 504 ของ Track123 มาถึงที่ ~6.15 วินาทีเสมอ (p50 6,147ms · p90 6,173ms —
 * ห่างกัน 26ms คือลายเซ็นของ gateway timeout ฝั่งเขา ไม่ใช่การกระจายตาม
 * ธรรมชาติ) การลองใหม่จึงมีราคาคงที่ ~6.5 วินาที และนั่นคือเหตุผลที่ผู้ใช้
 * รอ 14 วินาทีแล้วได้ error:
 *
 *   ไปรษณีย์ไทยตอบไม่พบ  1,700ms
 *   Track123 ครั้งที่ 1  6,147ms  ← 504
 *   หน่วงก่อนลองใหม่        400ms
 *   Track123 ครั้งที่ 2  6,147ms  ← 504 ซ้ำ
 *                      ─────────
 *                       14,394ms   (วัดได้จริง p50 13,989ms)
 *
 * อัตราการกู้สำเร็จจาก log จริง: **2 จาก 35 ครั้ง (~6%)** ที่เจอ 504 รอบแรก
 * แล้วรอบสองผ่าน ส่วนอีก 23 ครั้งเจอ 504 ซ้ำ
 *
 * ตัดสินใจแล้วว่ายอมเสีย 6% นั้น แลกกับให้อีก 94% เร็วขึ้นจาก 14.4 → 7.8 วินาที
 * (เร็วขึ้น 46%) และประหยัดการยิงรอบสองไปด้วย
 *
 * network_error ยังอยู่ในชุดนี้ เพราะมันคือ "ต่อไม่ติด" ซึ่งต่างกัน — ไม่มี
 * ราคาคงที่ 6 วินาทีแบบ 504 และมักหายเองในรอบถัดไป (timeout ถูกกันแยกอีกชั้น
 * ที่ isRetryableSystemError ดู TIMEOUT_UPSTREAM_CODE)
 * ------------------------------------------------------------------
 */
 ...
 * ------------------------------------------------------------------
 *
 * ⚠️ กติกาคือ "ไม่นับเมื่อไม่ได้ข้อมูลกลับมา" — สองกรณีที่ไม่นับ:
 *
 *   rate_limited    A0706 ปลายทางปฏิเสธตั้งแต่ยังไม่ประมวลผล
 *   upstream_error  ปลายทางรับไปแล้วแต่พังระหว่างทาง ไม่มีผลลัพธ์กลับมา
 *
 * ส่วนที่ **ยังนับ** เพราะถือว่าถูกคิดเงิน:
 *
 *   ok              ได้ข้อมูลจริง
 *   not_found       คำตอบจริงที่ปลายทางประมวลผลแล้ว ("ไม่มีเลขนี้" ก็คือคำตอบ)
 *   network_error   ไม่รู้ว่าคำขอไปถึงหรือยัง — นับไว้ก่อน ปลอดภัยกว่า
 *   auth_failed / config_error  เป็นปัญหาฝั่งเรา แต่คำขอออกไปแล้ว
 *
 * ⚠️ ตัวเลขนี้ยังเป็น **ค่าประมาณ** อยู่ดี เพราะเราไม่มีทางรู้กติกาการคิดเงิน
 * ที่แท้จริงของ Track123 จากฝั่งเรา วิธีเดียวที่จะจูนได้คือเทียบกับ dashboard
 * ของเจ้านั้นเป็นระยะ แล้วขยับเกณฑ์ตรงนี้ตาม
 */
function countsAgainstQuota(error: unknown): boolean {
  if (isRateLimited(error)) return false;
  // ปลายทางพังระหว่างประมวลผล = ไม่มีผลลัพธ์ให้คิดเงิน
  return !(error instanceof CarrierError && error.code === "upstream_error");
}
