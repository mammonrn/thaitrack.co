...
/**
 * เพดานเวลารอคำตอบจาก Track123 หนึ่งครั้ง
 *
 * ------------------------------------------------------------------
 * 10 วินาที ตั้งจากการวัดของจริง ไม่ใช่ตัวเลขกลมๆ
 *
 * กระจายของเวลาต่อ attempt ที่ gateway บันทึกไว้เอง (537 บรรทัด 1–4 ก.ย.):
 *
 *   result              n     p50      p90      max
 *   ok                 44   1,155ms  2,105ms   4,164ms
 *   not_found         261     656ms  2,703ms   9,107ms
 *   rate_limited      171      74ms     97ms     924ms
 *   upstream_error     61   6,147ms  6,173ms  13,889ms
 *
 * 10 วิ อยู่เหนือ max ของคำตอบที่ใช้ได้จริง (not_found 9,107ms) แบบพอมีที่หายใจ
 * — ต่ำกว่านี้จะไปตัดคำตอบจริงที่แค่มาช้าทิ้ง ซึ่งแย่กว่าการรอ
 *
 * เดิมตั้งไว้ 20 วินาที ซึ่งไม่เคยถูกแตะเลยสักครั้ง เพราะ Track123 มี gateway
 * timeout ของตัวเองที่ ~6.15 วิ (ดูความแน่นของ p50/p90 ของ upstream_error —
 * 6,147 กับ 6,173 ห่างกัน 26ms คือลายเซ็นของ timeout ฝั่งเขา ไม่ใช่ของเรา)
 * ค่า 20 วิจึงเป็นแค่ตาข่ายที่ห้อยอยู่เฉยๆ ไม่ได้กันอะไร
 *
 * ⚠️ ลดค่านี้ **ไม่ได้แก้** อาการรอ 14 วินาทีที่ผู้ใช้เจอ — ตัวนั้นคือการลองใหม่
 * หลังเจอ 504 (6.15 + 0.4 + 6.15 + ขาไปรษณีย์ไทย 1.7 ≈ 14.4 วิ) ค่านี้ช่วยแค่
 * ตัดหางที่ยาวผิดปกติ (attempt ที่กินไป 13,889ms จะถูกตัดที่ 10 วิ)
 * ------------------------------------------------------------------
 *
 * export ไว้ให้เทสต์ตรวจได้ว่ายังอยู่เหนือเพดานของคำตอบที่ใช้ได้จริง
 */
