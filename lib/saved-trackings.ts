 ...
 * เขียนไว้ต้องตรงกับขอบบนของแต่ละชั้นจริงๆ (ดู EXACT_MAX_METERS กับ
 * APPROXIMATE_MAX_METERS ใน lib/geocode.ts) ถ้าขยับเส้นแบ่งที่นั่น ต้องขยับ
 * ถ้อยคำตรงนี้ตาม
 *
 * ⚠️ coarse **ไม่มีขอบบนแล้ว** ตั้งแต่ตัดเพดาน 12 กม. ทิ้ง ถ้อยคำจึงต้องไม่
 * รับประกันระยะใดๆ — "หลายกิโลเมตร" ยังจริงทั้งกับ 3 กม. และ 40 กม. ส่วน
 * ประโยคแบบ "ไม่เกิน 12 กม." จะกลายเป็นคำโกหกทันทีที่เจอเคสที่ไกลกว่านั้น
 *
...
/**
 * นับสถานะจากรายการที่หน้าเว็บถืออยู่ตอนนั้น
 *
 * ฟังก์ชันนี้ไม่ยิงถามอะไรเองเลย มันนับสิ่งที่ส่งเข้ามาเท่านั้น — หน้าประวัติ
 * จึงต้องเรียกซ้ำหลังรีเฟรชเสร็จ (ดู app/history/history-list.tsx) ไม่งั้น
 * ตัวเลขสรุปด้านบนจะขัดกับการ์ดข้างล่างที่อัปเดตไปแล้ว
 *
...
export async function refreshSavedTrackings(
  ...
  try {
    ...
    return (rows as SavedTrackingRow[]).map(toSavedTracking);
  } catch {
    return [];
  }
}
